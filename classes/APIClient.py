...
class APIClient:
    ...
    def __init__(self, base_url=API_BASE_URL):
        ...
        self.timeout = (10, int(os.getenv("API_TIMEOUT", 60)))  # padrão 60s para timeout, configurável via .env
    ...
    def _request(self, method, endpoint, params=None, json=None, data=None):
        ...
        try:
            ...
            if response.status_code == 204:
                # return (json.dumps({"status": "success", "message": "Operação realizada com sucesso (No Content)"}, indent=4, ensure_ascii=False))
                return {"status": "success", "message": "Operação realizada com sucesso (No Content)"}
            
            try:
                return response.json()
            except requests.exceptions.JSONDecodeError as json_err:
                logger.error(f"(linha 91) Erro ao decodificar JSON da resposta: {json_err}")
            ...
            # logger.error(f"(linha 101) Resposta JSON: {json_module.dumps(error_data, indent=4, ensure_ascii=False)}")
            ...
            try:
                error_data = response.json()
                logger.debug(f"(linha 106) Resposta JSON: {json_module.dumps(error_data, indent=4, ensure_ascii=False)}")
                # return(json_module.dumps(error_data, indent=4, ensure_ascii=False))
                # Log().add("ERROR", f"(linha 110) Resposta JSON: {json_module.dumps(error_data, indent=4, ensure_ascii=False)}")
            except (requests.exceptions.JSONDecodeError, ValueError):
                print(f"Uallas Teste: {response.text[:500]}")
            ...
            return json_module.dumps(response.json(), indent=4, ensure_ascii=False)
    ...
    def patch(self, endpoint, params=None, data=None, json=None):
        return self._request('PATCH', endpoint, params=params, data=data, json=json)
