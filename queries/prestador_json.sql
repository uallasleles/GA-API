WITH CTE_FORNECEDOR AS (
    SELECT DISTINCT
        JSON_OBJECT(
            ...
            'HiredType' VALUE (CASE WHEN PCFORNEC.TIPOPESSOA = 'F' THEN 'Person' WHEN PCFORNEC.TIPOPESSOA = 'J' THEN 'Company' ELSE NULL END),
            'NationalId' VALUE NVL(PCFORNEC.CGCAUX, ''),
            ...
            'BrazilianSettings' VALUE JSON_OBJECT(
                ...
                'HiredPessoaJuridica' VALUE JSON_OBJECT(
                    ...
                    'OptanteSimplesNacional' VALUE (CASE WHEN PCFORNEC.SIMPLESNACIONAL = 'S' THEN 'true' ELSE 'false' END)
                )
            ),
