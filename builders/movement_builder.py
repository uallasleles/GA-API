...
from classes.db_queries import query_Movimento


def build_insert_payload(moviment: Dict[str, Any]) -> Dict[str, Any]:
    """Constrói o payload para inserir um novo movimento."""
    movement = load_Template("templates/Movement/movement.json")
    payload = load_Template("templates/Movement/MovementRegistration.json")

    try:
        payload["CodigoCliente"]        = movement["CODIGOCLIENTE"]
        payload["NomeMovimento"]        = movement["NOMEMOVIMENTO"]
        payload["TipoMovimento"]        = movement["TIPOMOVIMENTO"]
        payload["IncideSobreImposto"]   = movement["INCIDESOBREIMPOSTO"]
        payload["IncideSobreFrete"]     = movement["INCIDESOBREFRETE"]    
        payload["LiberaSaldo"]          = movement["LIBERASALDO"]
...
def build_update_payload(license: str) -> Dict[str, Any]:
    ...
    vehicle = query_Movimento(license)
